...
export const VwoFmeProvider: React.FC<VwoFmeProviderProps> = ({ options = {}, children }) => {
  ...
  useEffect(() => {
    const removeLogListener = VWO.registerLogCallback((log) => {
      ...
      setSdkLogs((prevLogs) => [...prevLogs, formattedLog]);
    });
    ...
    const initializeVWO = async () => {
      try {
        ...
        const removeIntegrationListener = VWO.registerIntegrationCallback((properties) => {
          console.log('Integration event received:', properties);
        });
        // Cleanup function
        vwoInstance.cleanup = () => {
  ...
  return (
    <VwoFmeContext.Provider value={{ vwo, isInitialized, error, sdkLogs }}>
      {children}

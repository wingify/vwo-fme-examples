...
class SmartBotRepositoryImpl(
    private val remoteDataSource: SmartBotRemoteDataSource
) : SmartBotRepository {

    override suspend fun sendChatQuery(userId: String, query: String): ChatResponse? {
        return remoteDataSource.sendChatQuery(userId, query)
    }

    override fun sendEvent(userId: String) {
        return remoteDataSource.sendEvent(userId)
    }
}